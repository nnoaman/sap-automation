  ...
  standard_ips                        = [
                                          {
                                            ...
                                            subnet_id = var.database.scale_out && var.database.database_dual_network_interfaces && var.database.high_availability ? (
                                              try(
                                                var.admin_subnet.id,
                                            ...
                                            private_ip_address = length(try(var.database.loadbalancer.frontend_ips[0], "")) > 0 ? (
                                              ...
                                              var.database.use_DHCP ? (
                                                null) : (
                                                cidrhost(
                                                  var.database.scale_out && var.database.database_dual_network_interfaces && var.database.high_availability ? var.admin_subnet.address_prefixes[0] : var.db_subnet.address_prefixes[0],
                                          ...
                                          },
                                          {
                                            ...
                                            subnet_id = var.database.scale_out && var.database.database_dual_network_interfaces  ? (
                                              try(
                                                var.admin_subnet.id,
                                                var.landscape_tfstate.admin_subnet_id
                                              )) : (
                                            ...
                                            private_ip_address = length(try(var.database.loadbalancer.frontend_ips[1], "")) > 0 ? (
                                              ...
                                              var.database.use_DHCP ? (
                                                null) : (
                                                cidrhost(
                                                  var.database.scale_out && var.database.database_dual_network_interfaces ? var.admin_subnet.address_prefixes[0] :  var.db_subnet.address_prefixes[0],
...
#######################################4#######################################8
  ...
  use_admin_nic_for_lb                = var.database.scale_out && var.database.database_dual_network_interfaces && var.database.high_availability

...
resource "azurerm_key_vault" "kv_user" {
  ...
  sku_name                             = "standard"
  rbac_authorization_enabled           = var.keyvault.enable_rbac_authorization
...
resource "azurerm_role_assignment" "role_assignment_msi" {
  ...
  count                                = var.keyvault.enable_rbac_authorization && var.options.assign_permissions ? 1 : 0
...
resource "azurerm_role_assignment" "role_assignment_vault_ssi" {
  provider                             = azurerm.deployer
  count                                = var.keyvault.enable_rbac_authorization && var.deployer_tfstate.deployer_uai.principal_id != data.azurerm_client_config.current.object_id ? 1 : 0
  scope                                = var.key_vault.user.exists ? (
                                           data.azurerm_key_vault.kv_user[0].id) : (
                                           azurerm_key_vault.kv_user[0].id
                                         )
...
resource "azurerm_role_assignment" "role_assignment_spn" {
  provider                             = azurerm.deployer
  count                                = var.keyvault.enable_rbac_authorization && var.options.assign_permissions && var.options.use_spn ? 1 : 0
  scope                                 = var.key_vault.user.exists ? (
                                           data.azurerm_key_vault.kv_user[0].id) : (
                                           azurerm_key_vault.kv_user[0].id
                                         )
...
resource "azurerm_role_assignment" "kv_user_msi_rbac" {
  ...
  count                               = !var.key_vault.user.exists && var.keyvault.enable_rbac_authorization && var.options.assign_permissions && (contains(keys(var.deployer_tfstate),"deployer_uai") ? length(var.deployer_tfstate.deployer_uai.principal_id) > 0 : false) ? 1 : 0
  scope                               = var.key_vault.user.exists ? (
                                           data.azurerm_key_vault.kv_user[0].id) : (
                                           azurerm_key_vault.kv_user[0].id
                                         )
...
resource "azurerm_role_assignment" "kv_user_msi_rbac_secret_officer" {
  provider                             = azurerm.deployer
  count                                = 0
  //count                                = !var.key_vault.user.exists && var.keyvault.enable_rbac_authorization && var.options.assign_permissions ? (
  //                                         0) : (
...
resource "azurerm_key_vault_access_policy" "kv_user_spn" {
  provider                             = azurerm.main
  count                                = !var.key_vault.user.exists && !var.keyvault.enable_rbac_authorization && var.options.use_spn ? 1 : 0
  key_vault_id                         = var.key_vault.user.exists ? (
                                           data.azurerm_key_vault.kv_user[0].id) : (
                                           azurerm_key_vault.kv_user[0].id
                                         )

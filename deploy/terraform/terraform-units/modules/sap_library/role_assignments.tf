resource "azurerm_role_assignment" "webapp_blob" {
  ...
  count                                = var.deployer.use && var.infrastructure.assign_permissions && length(var.deployer_tfstate.webapp_identity) > 0 && length(var.storage_account_tfstate.arm_id) == 0 ? 1 : 0
...
resource "azurerm_role_assignment" "webapp_table" {
  provider                             = azurerm.main
  count                                = var.deployer.use && var.deployer.use && var.infrastructure.assign_permissions && length(var.deployer_tfstate.webapp_identity) > 0 && length(var.storage_account_tfstate.arm_id) == 0 ? 1 : 0
  scope                                = azurerm_storage_account.storage_tfstate[0].id
...
resource "azurerm_role_assignment" "blob_msi" {
  provider                             = azurerm.main
  count                                = var.deployer.use && var.infrastructure.assign_permissions && length(var.deployer_tfstate.deployer_msi_id) > 0 && length(var.storage_account_tfstate.arm_id) == 0 ? 1 : 0
  scope                                = azurerm_storage_account.storage_tfstate[0].id
...
resource "azurerm_role_assignment" "dns_msi" {
  provider                             = azurerm.main
  count                                = var.deployer.use && var.infrastructure.assign_permissions && length(var.deployer_tfstate.deployer_msi_id) > 0 ? 1 : 0

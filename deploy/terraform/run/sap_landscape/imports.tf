...
data "azurerm_app_configuration_key" "deployer_state_file" {
  count                  = var.use_deployer ? length(trimspace(coalesce(var.application_configuration_id, try(data.terraform_remote_state.deployer[0].outputs.deployer_app_config_id, " ")), " ")) > 0 ? 1 : 0 : 0

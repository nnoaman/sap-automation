...
function sdaf_installer() {
	...
	if [ "${deployment_system}" == sap_deployer ]; then
		...
		if [ 1 == $called_from_ado ]; then
			# Check platform (GitHub or Azure DevOps)
			if [[ "${PLATFORM}" == "devops" ]]; then
				echo "Running in Azure DevOps environment - updating variable group"
				...
				echo "Running in GitHub Actions environment - setting environment variables"

				if [ -n "${app_config_id}" ]; then
					echo "APPLICATION_CONFIGURATION_ID=${app_config_id}" >> $GITHUB_ENV
					echo "Setting APPLICATION_CONFIGURATION_ID in GitHub environment"
					set_value_with_key "APPLICATION_CONFIGURATION_ID" "${app_config_id}" "env"
				fi

				if [ -n "${app_service_name}" ]; then
					echo "APPSERVICE_NAME=${app_service_name}" >> $GITHUB_ENV
					echo "Setting APPSERVICE_NAME in GitHub environment"
					set_value_with_key "APPSERVICE_NAME" "${app_service_name}" "env"
				fi

				if [ -n "${app_service_deployment}" ]; then
					echo "HAS_APPSERVICE_DEPLOYED=${app_service_deployment}" >> $GITHUB_ENV
					echo "Setting HAS_APPSERVICE_DEPLOYED in GitHub environment"
					set_value_with_key "HAS_APPSERVICE_DEPLOYED" "${app_service_deployment}" "env"
				fi

				if [ -n "${DEPLOYER_KEYVAULT}" ]; then
					echo "DEPLOYER_KEYVAULT=${DEPLOYER_KEYVAULT}" >> $GITHUB_ENV
					echo "Setting DEPLOYER_KEYVAULT in GitHub environment"

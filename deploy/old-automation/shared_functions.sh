...
"github")
    source ${SAP_AUTOMATION_REPO_PATH}/deploy/automation/platform/github_functions.sh
    ;;

"devops")
    source ${SAP_AUTOMATION_REPO_PATH}/deploy/automation/platform/devops_functions.sh
    ;;
...
function __appconfig_get_value_with_key() {
    ...
    var=$(az appconfig kv show -n ${APPLICATION_CONFIGURATION_NAME} --key ${key} --label ${CONTROL_PLANE_NAME} --query value --output tsv --auth-mode login)
...
function __appconfig_set_value_with_key() {
    ...
    var=$(az appconfig kv set -n ${APPLICATION_CONFIGURATION_NAME} --key ${key} --label ${CONTROL_PLANE_NAME} --value ${value} --content-type text/plain --yes  --auth-mode login)
...
function __appconfig_get_secret_with_key() {
    ...
    var=$(az appconfig kv show -n ${APPLICATION_CONFIGURATION_NAME} --key ${key} --label ${CONTROL_PLANE_NAME} --query value --secret --output tsv  --auth-mode login)

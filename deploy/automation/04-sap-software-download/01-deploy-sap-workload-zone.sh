...
command="ansible-playbook '-e "download_directory=/" -e "BOM_directory=${sample_path}" -e "bom_base_name=${bom_base_name}" -e "deployer_kv_name=${kv_name}" -e "check_storage_account=${re_download}"' $ExtraParams ${SAP_AUTOMATION_REPO_PATH}/deploy/ansible/playbook_bom_downloader.yaml"
